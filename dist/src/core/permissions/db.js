...
class PermissionDB {
    ...
    async updateApprovalStatus(id, status, approvedBy, reason) {
        if (this.isMock) {
        ...
        await this.pool.query('UPDATE approval_requests SET status = $1, approvedBy = $2, reason = $3, approvalTime = $4 WHERE id = $5', [status, approvedBy, reason, new Date().toISOString(), id]);
    }
    async addLead(email, tier) {
        if (this.isMock) {
            console.log(`[MockDB] Added lead: ${email} for tier ${tier}`);
            return;
        }
        await this.pool.query('INSERT INTO waitlist (email, tier) VALUES ($1, $2) ON CONFLICT (email) DO NOTHING', [email, tier]);
    }
